...
**目标**: 外部消费者和仓库维护者都只依赖公开导出与单一发布基线使用和发布 `@xinghunm/compass-ui`
**依赖**: 无（第一阶段）
**需求**: PUBL-01, PUBL-02, PUBL-03, PUBL-04, PUBL-05
**成功标准**（必须成立）:
...
**目标**: 外部用户可以从单一入口理解并验证 `compass-ui`，且文档、demo、测试都复用同一条真实消费链路
**依赖**: Phase 1
**需求**: DOCS-01, DOCS-02, DOCS-03, DOCS-04, DOCS-05
**成功标准**（必须成立）:
...
**目标**: 外部用户可以只依赖 `compass-ui` 完成常见表单输入、状态反馈、加载态与空态闭环
**依赖**: Phase 2
**需求**: COMP-01, COMP-02, COMP-03, COMP-04, COMP-05, COMP-06, COMP-07
**成功标准**（必须成立）:
...
**目标**: 复杂选择与 overlay 组件具备一致的键盘、焦点和关闭行为，并形成可持续维护的基础层策略
**依赖**: Phase 3
**需求**: INTR-01, INTR-02, INTR-03, INTR-04, INTR-05
**成功标准**（必须成立）:
...
| --------------------- | -------------- | ------ | --------- |
| 1. 公开契约与发布基线 | 0/TBD          | 未开始 | -         |
| 2. 文档入口与验证统一 | 0/TBD          | 未开始 | -         |
| 3. 基础组件补齐       | 0/TBD          | 未开始 | -         |
| 4. 复杂交互稳态化     | 0/TBD          | 未开始 | -         |
