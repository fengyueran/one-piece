...
- [ ] 把 `compass-ui` 作为当前阶段核心产品继续补强，优先补齐成熟组件库中的常用组件能力
- [ ] 提升 `compass-ui` 的文档完整度、demo 质量和对外可理解性，使其更适合作为公开 npm 包使用
- [ ] 明确 `compass-ui` 的组件覆盖优先级与演进路线，形成“先补齐常用能力，再考虑差异化”的节奏
- [ ] 持续打磨发布质量，确保核心包的导出面、测试、构建和版本发布流程足够稳定
- [ ] 评估 AI 时代下自有通用组件库的长期必要性，但这一判断应建立在真实使用与持续补齐常用能力之后，而不是当前提前收缩
...
| ------------------------------------------------------------------------------ | -------------------------------------------------- | -------- |
| 仓库定位为个人长期复用资产库，但产物默认按公开 npm 包标准建设                  | 既满足自己复用，又不把资产封闭在内部项目里         | — 待验证 |
| 当前阶段把 `compass-ui` 设为主线                                               | 现有包类型较多，必须先聚焦一个最能持续积累的核心包 | — 待验证 |
| `compass-ui` 先按“完整组件库，尽量自给自足”的方向推进                          | 你希望掌握完整可复用能力，而不是先做轻封装集合     | — 待验证 |
| 组件能力优先参考成熟框架的常用组件覆盖                                         | 先解决高频复用需求，比提前追求独特性更有价值       | — 待验证 |
| `ai-chat` 暂时保留，但不是当前优先投入对象                                     | 该能力已有基础沉淀，但当前主线不在这里             | — 待验证 |
| 本阶段明确不追求 Ant Design 级覆盖、不优先做移动端、不先做复杂设计系统平台能力 | 通过明确边界避免范围失控                           | — 待验证 |
