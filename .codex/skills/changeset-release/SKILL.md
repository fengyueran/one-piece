...
- 摘要默认使用“1 句总述 + 1 到 3 个 bullet”的结构；不要把多个变化点硬塞进一整句长句里。
- 总述用于概括本次变更的主题，bullet 用于拆开具体变化点；如果只有一个清晰变化点，也可以只写一个简短总述。
- 每个 bullet 只描述一个用户可感知变化，优先用“支持……”“修复……”“调整……”开头。
- 优先写行为变化、交互变化、视觉变化、API 变化；不要把实现细节、重构动作、测试补充直接写成对外价值。
- 避免空泛词，例如“能力”“表现”“体验优化”“相关问题修复”；要直接写清楚具体是什么变了。
- 如果一次变更同时包含交互修复和样式修复，必须拆成两个 bullet，不能并列塞在同一句里。
- 如果变更本质上只是文档、示例、测试或内部整理，就直接写“补充文档示例”“更新测试覆盖”“整理内部实现”，不要包装成组件新增特性。

推荐模板：

```md
---
'@scope/pkg-name': patch
---
...
```md
修复 Select 组件下拉菜单的受控展开能力，并调整选中项文字与勾选图标之间的间距表现。
```
