...
Popconfirm 还会继承 `Popover`、`Button` 以及全局 `colors.text`、`colors.textSecondary`、`fontSize.md`、`fontSize.sm` 与 `fontWeight.semibold` 等 token 表现，但组件级覆盖优先使用 `components.popconfirm.*`。
