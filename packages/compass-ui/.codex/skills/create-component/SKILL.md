...
- 如果组件公开了 `classNames` / `styles` 语义化插槽，必须在 API 表格后补 `### classNames / styles 插槽`，明确列出所有 slot key
...
- `## 主题变量 (Design Token)` 必须是组件文档最后一个二级章节
- token 表格统一使用 `Token Name` / `Description` / `Default`
- 默认值应与 `src/theme/default-theme.ts` 保持一致
- token 数量较少时直接展示完整表格
- token 数量明显较多时，优先展示一张“高频 token”表，再用折叠块补“完整 token 列表”
- 折叠块只能作为补充，不能把唯一的 token 信息全部藏起来
- 组件如果同时受全局 token 影响，必须在表格后补一行简短说明，不再使用旧的 `<details>`、`组件 Token`、`全局 Token` 块
