import styled from '@emotion/styled'
import { TabsProps } from './types'
import { getComponentTheme, getThemeToken } from '../theme/utils'

export const TabsContainer = styled.div<{ $tabPosition?: TabsProps['tabPosition'] }>`
  display: flex;
  flex-direction: ${({ $tabPosition }) =>
    $tabPosition === 'left' || $tabPosition === 'right' ? 'row' : 'column'};
  width: 100%;
  overflow: hidden;

  ${({ $tabPosition }) =>
    $tabPosition === 'bottom' &&
    `
    flex-direction: column-reverse;
  `}

  ${({ $tabPosition }) =>
    $tabPosition === 'right' &&
    `
    flex-direction: row-reverse;
  `}
`

export const TabsNav = styled.div<{
  $tabPosition?: TabsProps['tabPosition']
  $type?: TabsProps['type']
}>`
  display: flex;
  flex: none;
  position: relative;

  ${({ $tabPosition, $type, theme }) => {
    const tabsTheme = getComponentTheme(theme, 'tabs')
    const border = `${tabsTheme.tabBarBorderWidth} solid ${tabsTheme.tabBarBorderColor}`

    // Only apply border if type is line
    if ($type !== 'line') {
      return `
        border-bottom: none;
        border-top: none;
        border-right: none;
        border-left: none;
      `
    }

    if ($tabPosition === 'top' || !$tabPosition) return `border-bottom: ${border};`
    if ($tabPosition === 'bottom') return `border-top: ${border};`
    if ($tabPosition === 'left') return `border-right: ${border};`
    if ($tabPosition === 'right') return `border-left: ${border};`

    return ''
  }}

  margin-bottom: ${({ $tabPosition }) => ($tabPosition === 'top' || !$tabPosition ? '16px' : '0')};
  margin-top: ${({ $tabPosition }) => ($tabPosition === 'bottom' ? '16px' : '0')};
  margin-right: ${({ $tabPosition }) => ($tabPosition === 'left' ? '16px' : '0')};
  margin-left: ${({ $tabPosition }) => ($tabPosition === 'right' ? '16px' : '0')};

  background-color: ${({ theme }) => getComponentTheme(theme, 'tabs').tabBarBackgroundColor};

  ${({ $tabPosition }) =>
    ($tabPosition === 'left' || $tabPosition === 'right') &&
    `
    flex-direction: column;
    height: 100%;
    overflow: hidden;
  `}

  ${({ $tabPosition }) =>
    ($tabPosition === 'top' || $tabPosition === 'bottom' || !$tabPosition) &&
    `
    width: 100%;
    overflow: hidden;
  `}
`

export const TabsNavWrap = styled.div<{ $tabPosition?: TabsProps['tabPosition'] }>`
  display: flex;
  flex: auto;
  align-self: flex-start;
  overflow: hidden;
  white-space: nowrap;
  transform: translate(0);

  ${({ $tabPosition }) =>
    $tabPosition === 'left' || $tabPosition === 'right'
      ? `
    flex-direction: column;
    overflow-y: auto;
    overflow-x: hidden;
  `
      : `
    overflow-x: auto;
    overflow-y: hidden;
  `}

  &::-webkit-scrollbar {
    width: 6px;
    height: 6px;
  }

  &::-webkit-scrollbar-thumb {
    background-color: rgba(0, 0, 0, 0.1);
    border-radius: 3px;
  }

  &::-webkit-scrollbar-track {
    background-color: transparent;
  }
`

export const TabsNavList = styled.div<{ $tabPosition?: TabsProps['tabPosition'] }>`
  display: flex;
  position: relative;
  transition: transform 0.3s;
  ${({ $tabPosition }) =>
    ($tabPosition === 'left' || $tabPosition === 'right') &&
    `
    flex-direction: column;
    width: 100%;
  `}
`

export const TabItem = styled.div<{
  $active?: boolean
  $disabled?: boolean
  $size?: TabsProps['size']
  $position?: TabsProps['tabPosition']
  $type?: TabsProps['type']
}>`
  position: relative;
  display: inline-flex;
  align-items: center;

  ${({ theme, $position }) => {
    const tabsTheme = getComponentTheme(theme, 'tabs')
    return `padding: ${tabsTheme.tabItemPadding};
    margin: ${
      $position === 'left' || $position === 'right'
        ? `0 0 ${tabsTheme.tabItemVerticalGutter} 0`
        : `0 ${tabsTheme.tabItemHorizontalGutter} 0 0`
    };`
  }}

  font-size: ${({ theme, $size }) => {
    const tabsTheme = getComponentTheme(theme, 'tabs')
    const defaultSize = tabsTheme.tabItemFontSize
    if ($size === 'large') return `calc(${defaultSize} + 2px)`
    if ($size === 'small') return `calc(${defaultSize} - 2px)`
    return defaultSize
  }};
  background: transparent;
  border: 0;
  outline: none;
  cursor: ${({ $disabled }) => ($disabled ? 'not-allowed' : 'pointer')};

  ${({ theme, $active, $disabled }) => {
    const tabsTheme = getComponentTheme(theme, 'tabs')
    if ($disabled) return `color: ${tabsTheme.tabItemDisabledColor};`
    if ($active) return `color: ${tabsTheme.tabItemActiveColor};`
    return `color: ${tabsTheme.tabItemColor};`
  }}

  font-weight: ${({ theme }) => getThemeToken(theme, 'fontWeight').normal};
  transition: all 0.3s;

  &:hover {
    ${({ theme, $disabled }) => {
      const tabsTheme = getComponentTheme(theme, 'tabs')
      return !$disabled ? `color: ${tabsTheme.tabItemHoverColor};` : ''
    }}
  }

  ${({ $type, $active, theme }) => {
    if ($type !== 'card' && $type !== 'editable-card') return ''
    const colors = getThemeToken(theme, 'colors')
    return `
    margin: 0 2px 0 0;
    padding: 8px 16px;
    background: ${$active ? '#ffffff' : '#fafafa'};
    border: 1px solid ${colors.border};
    border-bottom: ${$active ? '1px solid #ffffff' : `1px solid ${colors.border}`};
    border-radius: 4px 4px 0 0;
    transition: all 0.3s cubic-bezier(0.645, 0.045, 0.355, 1);
  `
  }}
`

export const CloseButton = styled.span`
  display: inline-flex;
  align-items: center;
  justify-content: center;
  margin-left: 8px;
  font-size: 12px;
  width: 16px;
  height: 16px;
  line-height: 0;
  border-radius: 50%;
  color: ${({ theme }) => getThemeToken(theme, 'colors').textSecondary};
  transition: all 0.2s;

  &:hover {
    background-color: rgba(0, 0, 0, 0.06);
    color: ${({ theme }) => getThemeToken(theme, 'colors').text};
  }
`

export const InkBar = styled.div<{
  $left: number
  $width: number
  $top: number
  $height: number
  $position?: TabsProps['tabPosition']
  $type?: TabsProps['type']
}>`
  position: absolute;
  background: ${({ theme }) => getComponentTheme(theme, 'tabs').inkBarColor};
  pointer-events: none;
  transition:
    width 0.3s,
    left 0.3s,
    top 0.3s,
    height 0.3s;

  display: ${({ $type }) => ($type === 'line' ? 'block' : 'none')};

  ${({ $position, $left, $width, $top, $height, theme }) => {
    const barHeight = getComponentTheme(theme, 'tabs').inkBarHeight
    if ($position === 'left' || $position === 'right') {
      return `
        width: ${barHeight};
        height: ${$height}px;
        top: ${$top}px;
        ${$position === 'left' ? 'right: 0;' : 'left: 0;'}
      `
    }
    return `
      height: ${barHeight};
      width: ${$width}px;
      left: ${$left}px;
      ${$position === 'bottom' ? 'top: 0;' : 'bottom: 0;'}
    `
  }}
`

export const TabsContent = styled.div`
  display: flex;
  flex: auto;
  flex-direction: column;
  min-width: 0;
  min-height: 0;
`

export const TabPaneContainer = styled.div<{ $active: boolean }>`
  flex: none;
  width: 100%;
  outline: none;
  display: ${({ $active }) => ($active ? 'block' : 'none')};
`
