...
export class DicomViewControllers extends engine.TSBehaviour {
  crosshairData?: components.image.CrosshairData;
  ...
  setupData(
    ...
    const panCrosshair = this.gameObject.addComponent(components.PanCrosshair);
    ...
    const panZoom = this.gameObject.addComponent(components.PanZoom);
    panZoom.mousebuttonLimit = 0;
    panZoom.zoomer = cameraZoom;

    this.panops[PanType.Zoom] = panZoom;

    const wheelZoom = this.gameObject.addComponent(components.WheelZoom);
    wheelZoom.zoomer = cameraZoom;
    this.wheelops[WheelType.Zoom] = wheelZoom;

    const wheelSlice = this.gameObject.addComponent(components.WheelSlice);
    ...
    const tapCrosshair = this.gameObject.addComponent(components.TapCrosshair);
