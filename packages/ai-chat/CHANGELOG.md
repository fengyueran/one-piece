...
- 改进聊天线程在流式回复中的自动滚动与回到底部交互：
  - 修复消息流式更新和结束时的自动贴底行为，确保最新内容能稳定滚动到可见区域。
  - 当用户离开底部查看历史时显示“回到底部”按钮，避免后续更新强制打断阅读。
  - 调整“回到底部”按钮的悬浮呈现方式，避免连续上滚后按钮消失。
...
- 修复了 plan 模式问卷卡片的稳定标识与提交流程。现在同一请求下的多个计划问题可以同时显示，不会互相覆盖；带稳定 `blockKey` 的问卷更新会正确替换原卡片，并在提交时命中对应的待处理选项。
