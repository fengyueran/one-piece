...
(function (root, factory) {
  ...
  var dicomParser = (function (dicomParser) {
    ...
    dicomParser.parseDicom = function (byteArray, options) {
      ...
      function readTransferSyntax(metaHeaderDataSet) {
        ...
        return dicomParser.readFixedString(
          byteArray,
      ...
      function getDataSetByteStream(transferSyntax, position) {
        if (transferSyntax === '1.2.840.10008.1.2.1.99') {
          ...
          return new dicomParser.ByteStream(
      ...
      function readDataSet(metaHeaderDataSet) {
        ...
        var elements = {};
        var dataSet = new dicomParser.DataSet(
        ...
        try {
          if (explicit) {
            dicomParser.parseDicomDataSetExplicit(
              dataSet,
              dataSetByteStream,
            ...
            dicomParser.parseDicomDataSetImplicit(
              dataSet,
              dataSetByteStream,
              dataSetByteStream.byteArray.length,
              options,
  ...
  var dicomParser = (function (dicomParser) {
    ...
    function getPixelDataFromFragments(byteStream, fragments, bufferSize) {
      ...
      if (fragments.length === 1) {
        return new Uint8Array(
          byteStream.byteArray.buffer,
          fragments[0].dataOffset,
          fragments[0].length,
        );
      }
    ...
    function readFragmentsUntil(byteStream, endOfFrame) {
      ...
      var bufferSize = 0;
      while (
        byteStream.position < endOfFrame &&
        byteStream.position < byteStream.byteArray.length
      ) {
  ...
  var dicomParser = (function (dicomParser) {
    ...
    dicomParser.DataSet.prototype.numStringValues = function (tag) {
      var element = this.elements[tag];
      if (element && element.length > 0) {
        var fixedString = dicomParser.readFixedString(
          this.byteArray,
          element.dataOffset,
          element.length,
        );
  ...
  var dicomParser = (function (dicomParser) {
     ...
     * @param element
     */
    dicomParser.findItemDelimitationItemAndSetElementLength = function (
      byteStream,
      element,
      warnings,
    ) {
        ...
        if (groupNumber === 0xfffe) {
          ...
          if (elementNumber === 0xe00d) {
            ...
            if (itemDelimiterLength === 0xffffffff) {
              ...
              if (warnings) {
                element.dataSet = dicomParser.readDicomDataSetExplicitUndefinedLength(
                  byteStream,
                  warnings,
                );
              } else {
  ...
  var dicomParser = (function (dicomParser) {
    ...
    dicomParser.parseDicomDataSetExplicit = function (dataSet, byteStream, maxPosition, options) {
      ...
      while (byteStream.position < maxPosition) {
        var element = dicomParser.readDicomElementExplicit(
          byteStream,
          dataSet.warnings,
          options.untilTag,
        );
    ...
    dicomParser.parseDicomDataSetImplicit = function (dataSet, byteStream, maxPosition, options) {
      ...
      while (byteStream.position < maxPosition) {
        var element = dicomParser.readDicomElementImplicit(
          byteStream,
          options.untilTag,
          options.vrCallback,
        );
  ...
  var dicomParser = (function (dicomParser) {
    ...
    dicomParser.readPart10Header = function (byteArray, options) {
      ...
      var littleEndianByteStream = new dicomParser.ByteStream(
        dicomParser.littleEndianByteArrayParser,
        byteArray,
      );
      ...
      function readTheHeader() {
        ...
        while (littleEndianByteStream.position < littleEndianByteStream.byteArray.length) {
          ...
          if (element.tag == 'x00020000') {
            if (element.length == 2) {
              headMaxPosition = dicomParser.littleEndianByteArrayParser.readUint16(
        ...
        var metaHeaderDataSet = new dicomParser.DataSet(
  ...
  var dicomParser = (function (dicomParser) {
    ...
    function readDicomDataSetExplicitUndefinedLength(byteStream, warnings) {
      ...
      while (byteStream.position < byteStream.byteArray.length) {
        ...
        if (element.tag === 'xfffee00d') {
          return new dicomParser.DataSet(
    ...
    function readSequenceItemExplicit(byteStream, warnings) {
      ...
      if (item.length === 4294967295) {
        ...
        item.dataSet = new dicomParser.DataSet(
          byteStream.byteArrayParser,
          byteStream.byteArray,
          {},
        );
