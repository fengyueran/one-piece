...
function decode8(imageFrame: ImageFrame, pixelData: Uint8Array) {
...
function decode8Planar(imageFrame: ImageFrame, pixelData: Uint8Array) {
  const frameData = pixelData;
  const frameSize = imageFrame.rows * imageFrame.columns;
  const outFrame = new ArrayBuffer(frameSize * imageFrame.samplesPerPixel);
  const header = new DataView(frameData.buffer, frameData.byteOffset);
...
function decode16(imageFrame: ImageFrame, pixelData: Uint8Array) {
  const frameData = pixelData;
  const frameSize = imageFrame.rows * imageFrame.columns;
...
export const decodeRLE = (imageFrame: ImageFrame, pixelData: Uint8Array): ImageFrame => {
