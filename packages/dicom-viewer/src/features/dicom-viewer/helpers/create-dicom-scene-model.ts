...
export const createDicomSceneModel = (
  type: Plane,
  image: Image3D,
  mask?: ccloader.image.Image3D<Uint8Array>,
  option?: Option,
) => {
  let dicomSceneModel;
  ...
  switch (type) {
    case Plane.Axial: {
      dicomSceneModel = new DicomSceneModel(
        'axial',
        image.getAxialInfo(),
        ccloader.image.makeAxialLoader(image),
        mask && ccloader.image.makeAxialLoader(mask),
        exampleColorTable,
        orthographicSize,
        option,
      );
...
export const getBasicInfo = (
  image: ccloader.image.Image3D<Int16Array> | ccloader.dicom.Image3DFromDicom<Int16Array>,
