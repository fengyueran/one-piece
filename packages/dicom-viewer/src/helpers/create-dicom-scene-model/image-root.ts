...
export class DicomImageRoot extends products.common.components.DicomImageRoot {
  ...
  createController(zoom?: number, zoomRange?: [number, number]): void {
    ...
    this.inputGo = this.scene.createGameObject(
      `${this.gameObject.name}/input`,
      this.imageRoot
    );
  ...
  addMinimap(): void {
    ...
    const basePlane = this.baseGo.getComponent(
      components.image.DicomImagePlane
    );
    if (!basePlane) {
      return;
    }
    const minimap = this.scene
      .createGameObject('minimap')
      .addComponent(components.minimap.MiniMap);
