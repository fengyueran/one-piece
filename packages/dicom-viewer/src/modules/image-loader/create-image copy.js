...
import decodeImageFrame from './decode-image-frame.js';
...
function createImage(imageId, pixelData, transferSyntax, options = {}) {
  ...
  options.preScale = {
    enabled:
      options.preScale && options.preScale.enabled !== undefined ? options.preScale.enabled : false,
  };
    ...
    decodePromise.then(function (imageFrame) {
      ...
      if (isColorImage) {
        if (useRGBA) {
          ...
          const colorBuffer = new Uint8ClampedArray((imageFrame.pixelData.length / 4) * 3);
          ...
          const colorBuffer = new imageData.data.constructor((imageData.data.length / 4) * 3);
      ...
      const image = {
        ...
        voiLUTFunction: voiLutModule.voiLUTFunction ? voiLutModule.voiLUTFunction : undefined,
